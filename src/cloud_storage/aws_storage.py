...
class SimpleStorageService:
    ...
    def __init__(self):
        ...
        """
        s3_client = S3Client()
        self.s3_resource: boto3.resources.base.ServiceResource = s3_client.s3_resource  # type: ignore
    ...
    @staticmethod
    def read_object(s3_object, decode: bool = True, make_readable: bool = False) -> Union[StringIO, str]:
        """
        Reads the specified S3 object with optional decoding and formatting.

        Args:
            s3_object: The S3 object (not a string key).
    ...
    def get_bucket(self, bucket_name: str) -> Bucket:
        ...
        try:
            if self.s3_resource is None:
                raise MyException(Exception("S3 resource is not initialized"), sys)
    ...
    def load_model(self, model_name: str, bucket_name: str, model_dir: Optional[str] = None) -> object:
        ...
        try:
            ...
            # Ensure file_object is a single S3 object, not a list
            if isinstance(file_object, list):
                if not file_object:
                    raise MyException(Exception("No file objects found for the given model file."), sys)
                file_object = file_object[0]
            # Read raw bytes from the S3 object for pickle loading
            model_obj = file_object.get()["Body"].read() # type: ignore
